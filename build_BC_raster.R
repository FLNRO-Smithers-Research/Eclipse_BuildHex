...
bc_DEM <- raster("D:/CommonTables/DEMs/BC_400m_DEM.tif")
projectRaster(bc_DEM, crs=crs(CRS.WGS84))

#extracts elevation for each point in p
coords$elev <- raster::extract(bc_DEM,bc.pt2)
...
write.csv (pts.info2,  "./outputs/BC_400m_GridPts.csv", row.names = FALSE) 
...
write.csv (pts.info2,  "./outputs/BC_BGC_400m_GridPts.csv", row.names = FALSE) ### this file to be submitted to ClimateWNA
